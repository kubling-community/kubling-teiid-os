...
import com.kubling.core.util.FilesystemHelper;
        ...
        if (stream == null) {
            ...
            try {
                ...
                try {
                    ...
                    final var ksFo = FilesystemHelper.getManager().resolveFile(name);
                    if (!ksFo.exists() || !ksFo.isFile() || ksFo.getContent().isEmpty())
                        throw new IOException(JDBCPlugin.Util.getString("SocketHelper.keystore_not_found", name));

                    stream = ksFo.getContent().getInputStream();

                } catch (FileSystemException fileSystemException) {
                    StringWriter sw = new StringWriter();
                    PrintWriter pw = new PrintWriter(sw);
                    fileSystemException.printStackTrace(pw);
                    throw new IOException(JDBCPlugin.Util.getString("SocketHelper.keystore_not_found", name), fileSystemException);
                }
