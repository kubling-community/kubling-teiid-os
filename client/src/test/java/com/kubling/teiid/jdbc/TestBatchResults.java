...
public class TestBatchResults {
    ...
    static class MockBatchFetcher implements BatchResults.BatchFetcher {
        ...
        public BatchResults.Batch requestBatch(int beginRow) throws SQLException {
            ...
            BatchResults.Batch batch = new BatchResults.Batch(createBatch(beginRow, endRow), beginRow, endRow);
            if (last || useLastRow) {
    ...
    BatchResults getBatchResults(List<?>[] batch, boolean isLast) {
        BatchResults.Batch batch2 = new BatchResults.Batch(batch, 1, batch.length);
        if (isLast) {
