...
public final class JsonFlattener {
    ...
    /**
     * {ROOT} is the default key of the Map returned by {@link #flattenAsMap}. When
     ...
     * in the Map with {ROOT} as its key.
     */
     ...
     * separator is a dot(.).
     *
     * @param separator any CharSequence
     * @return this {@link JsonFlattener}
     */
    public JsonFlattener withSeparator(CharSequence separator) {
     ...
     * <br>
     * {@link com.kubling.teiid.core.json.unflattener.JsonUnflattener} may cause unpredictable results with the JSON produced by a
    ...
    private void reduce(JsonValueBase<?> val) {
        if (val.isObject() && val.asObject().iterator().hasNext()) {
            elementIters.add(IndexedPeekIterator.newIndexedPeekIterator(val.asObject()));
            ...
            switch (flattenMode) {
                case KEEP_PRIMITIVE_ARRAYS:
                    ...
                    if (allPrimitive) {
                        ...
                        elementIters.add(IndexedPeekIterator.newIndexedPeekIterator(val.asArray()));
                    }
                    break;
                case KEEP_ARRAYS:
                    JsonifyArrayList<Object> array = newJsonifyArrayList();
    ...
    private Object jsonVal2Obj(JsonValueBase<?> val) {
        ...
        if (Objects.requireNonNull(flattenMode) == FlattenMode.KEEP_ARRAYS) {
            if (val.isArray()) {
                JsonifyArrayList<Object> array = newJsonifyArrayList();
                for (JsonValueBase<?> value : val.asArray()) {
                    array.add(jsonVal2Obj(value));
    ...
    private boolean hasReservedCharacters(String key) {
        if (flattenMode.equals(FlattenMode.MONGODB) && StringUtils.containsAny(key, separator))
            throw new IllegalArgumentException(
                    "Key cannot contain separator(" + separator + ") in FlattenMode." + FlattenMode.MONGODB);

        return (StringUtils.contains(key, separator) || StringUtils.containsAny(key, leftBracket, rightBracket));
    }
    ...
    private String computeKey() {
        ...
        for (IndexedPeekIterator<?> iter : elementIters) {
            if (iter.getCurrent() instanceof Entry) {
                ...
                sb.append(flattenMode.equals(FlattenMode.MONGODB) ? separator : leftBracket);
                sb.append(iter.getIndex());
                sb.append(flattenMode.equals(FlattenMode.MONGODB) ? "" : rightBracket);
            }
        }
